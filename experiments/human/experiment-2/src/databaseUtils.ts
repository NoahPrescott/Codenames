...
async function selectWordPairsForExperiment() {
  ...
  const wordPairsRef = collection(db, 'human-clue-word-pairs'); // for human clue source
...
async function incrementCount(docId: string) {
  ...
  const wordPairDocRef = doc(db, 'human-clue-word-pairs', docId); // Reference to the document
...
async function initData(userInfo: UserRecord): Promise<void> {
  ...
  const exptDataDoc = getDocStr('exptDataHumanClue')
