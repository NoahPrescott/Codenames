...
// ./databaseUtils should grab Human Clue word pairs -- redundancy and grossness in code
...
const exitExperiment = () => {
  // Define the HTML for the exit message and "Continue" button
  document.body.innerHTML = `
      <div style="text-align: center; padding: 20px;">
          <p>Thank you. Click 'Continue' to return to Prolific.</p>
          <p>Please use the following completion code to ensure compensation for this study:</p>
          <p><strong>${prolificCC}</strong></p>
          ${debuggingText || ''}
          <br /><br />
          <button style="padding: 10px 20px; font-size: 16px;" onclick="window.location.replace('${prolificCUrl}')">Continue</button>
      </div>
  `;
};
...
export async function runExperiment() {
  ...
  var nTrials = 80; // should ideally be length of word pairs; 80 word pairs are selected and generated in databaseUtil.ts
  ...
  var trial = {
      ...
      stimulus: function() {
        ...
        return `
          <div style="margin-bottom: 40px; font-weight: bold; font-size: 1.2em;">
            Please indicate how related these two words are.
          </div>
          <div style="display: flex; justify-content: center; gap: 60px; margin-bottom: 80px;">
            <span style="border: 1px solid black; padding: 10px; font-size: 1.1em;">${word1}</span>
            <span style="border: 1px solid black; padding: 10px; font-size: 1.1em;">${word2}</span>
          </div>
          `;
      },
      prompt: "", 
